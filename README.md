...
从你提供的网页内容来看，**15道单选题中仅完整展示了8道题的题干（第1-8题），其余9-15题的题干、选项均未明确给出**（网页提示“限于篇幅，其余选择题和解析请到网站上查看”）。因此，以下仅针对可获取的第1-8题，结合AI方向校招高频考点及题干信息推导答案与详细解析（注：部分题目因原始网页存在公式/符号缺失，需基于同类题型补全逻辑）。
...
1. 明确已知条件：  
   - $x$的绝对误差限$\epsilon(x) = 0.0005$（精确到小数点后3位，即$x \in [1.2335, 1.2345]$）；  
   - $y$的绝对误差限$\epsilon(y) = 0.0005$（精确到小数点后3位，即$y \in [0.5665, 0.5675]$）。  

2. 计算$x + y$的绝对误差限：  
   $\epsilon(x + y) = \epsilon(x) + \epsilon(y) = 0.0005 + 0.0005 = 0.001$，对应选项A。  

   若题干为其他运算（如$x \times y$），则需用乘法误差限公式$\epsilon(xy) \leq |x|\epsilon(y) + |y|\epsilon(x)$，但基于“绝对误差限”的基础考点，优先按“和的误差限”推导。  


### 第7题：逻辑回归的优化方法与局部最优
#### 题干：
在逻辑回归中，若目标函数为交叉熵损失函数$L(\theta) = -\frac{1}{m}\sum_{i=1}^m [y_i\log h_\theta(x_i) + (1-y_i)\log(1-h_\theta(x_i))]$（其中$h_\theta(x) = \frac{1}{1+e^{-\theta^T x}}$），以下哪种优化方法最可能陷入局部最优？  
选项：  
A. 拟牛顿法（BFGS）  
B. 高斯 - 牛顿法  
C. 随机梯度下降（SGD）  
...
#### 答案：C
#### 推理过程：
核心是**逻辑回归损失函数的凸性**与**优化方法的特性**：  
1. 首先明确：逻辑回归的交叉熵损失函数是**严格凸函数**（二阶导数Hessian矩阵正定），理论上“凸函数无局部最优，仅有全局最优”——但这是“批量优化”（用全量数据）的前提，当使用“随机/小批量数据”时，情况会变化。  

2. 逐一分析选项：  
   - A. 拟牛顿法（BFGS）：批量优化方法，利用全量数据估计Hessian矩阵的逆，收敛到全局最优（凸函数下），几乎不陷局部最优。  
   - B. 高斯-牛顿法：主要用于非线性最小二乘问题（如岭回归），逻辑回归中较少用，且批量计算时也收敛到全局最优。  
   - C. 随机梯度下降（SGD）：每次用单个样本更新参数，损失函数的“随机梯度”是真实梯度的无偏估计，但存在**噪声**。在迭代过程中，噪声可能导致参数在“全局最优附近震荡”，甚至在非凸场景（如特征非线性映射后）陷入局部最优——即使逻辑回归本身是凸的，SGD的随机性也可能使其暂时停留在“局部极小值附近”（虽理论上是全局最优，但实际中更易出现此类现象）。  
   - D. 牛顿法：批量优化方法，直接利用Hessian矩阵（正定）进行二次逼近，收敛速度快，且凸函数下必收敛到全局最优，不会陷局部最优。  

3. 关键结论：在凸函数场景下，**批量优化方法（牛顿法、BFGS）必收敛到全局最优**，而**随机梯度下降（SGD）因随机性，可能因噪声或步长问题表现出“局部最优”的特征**，因此选C。  


### 第8题：RAG系统检索失败的原因分析
#### 题干：
一个团队构建的RAG问答系统在测试中表现不佳。用户提问：“在B-21轰炸机的研发合同中，诺斯罗普·格鲁曼公司承接的初始作战能力（IOC）目标年份是哪一年？”系统未能找到答案，尽管知识库中的一份文档里明确写着：“……该项目的主要承包商是诺斯罗普·格鲁曼公司。根据合同条款，B-21轰炸机的初始作战能力目标设定在2027年。”工程师排查后发现，文档切分时，恰好将这两句话分在了两个相邻但独立的文本块（chunk）中：  
...
#### 答案：C
#### 推理过程：
逐一排除选项，核心是**RAG系统的“文档切分→Embedding→检索”流程**：  
1. 选项A错误：题干未提及“专有名词分割错误”（如“诺斯罗普·格鲁曼”被拆成“诺斯罗普”和“格鲁曼”），且即使Tokenizer不包含该词，也会通过Subword（如BPE）拆分，仍能生成有意义的向量，不会直接导致检索失败。  
2. 选项B错误：Embedding模型的核心能力是“捕捉实体关联”，但前提是“关联信息在同一文本块中”。若Chunk B中没有“诺斯罗普·格鲁曼”，模型无法凭空建立“Chunk B与该公司”的关联——这不是模型质量问题，而是输入数据（文本块）缺失关联信息导致的。  
3. 选项C正确：这是RAG中“文档切分（Chunking）”的经典问题——**“切分粒度不当导致上下文断裂”**。用户查询同时包含“B-21轰炸机”“诺斯罗普·格鲁曼”“年份”三个关键信息，但：  
   - Chunk A只有“诺斯罗普·格鲁曼”，无“B-21”和“年份”，无法与查询匹配；  
   - Chunk B只有“B-21”和“年份”，无“诺斯罗普·格鲁曼”，无法与查询中的“该公司承接的合同”建立关联；  
   根本原因是切分策略破坏了“承包商-项目-年份”的上下文完整性，导致Embedding向量无法捕捉三者的关联。  
4. 选项D错误：题干未提及“海量向量”或“ANN算法精度”（如FAISS的IVF算法参数不当），且即使ANN精度低，若Chunk B的向量与查询向量足够相似，仍有概率被检索到——此次失败的核心是“Chunk B的向量与查询向量不相似”（因缺失关键实体），而非检索算法精度问题。  
